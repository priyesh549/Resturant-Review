...
function Resturant() {
  ...
  return (
    <div>
      ...
      <span style={{fontWeight:'54px',marginRight:'3px'}}><strong>Pages-></strong></span>
      {
        page > 0 && (new Array(page)).fill(0).map((num, index) => <Button variant='outline-primary' onClick={()=>setcurrpage(index+1)}>{index+1}</Button>)
      }
    </div>
  );
}
